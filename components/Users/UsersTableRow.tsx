...
export default function UsersTableRow(user: UsersTableRowProp) {
  return (
    <Link href={`/users/${user.id}`} className={ 'flex justify-between items-center px-2 cursor-pointer text-gray-600 hover:bg-emerald-100 duration-100' + (user.index % 2 !== 0 ? ' bg-slate-200/70' : '')}>
