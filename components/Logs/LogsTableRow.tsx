...
export default function LogsTableRow(log: LogsTableRowProp) {
  ...
  return (
    <div onClick={ () => log.userId &&  router.push(`/users/${log.userId}`) } className={ 'flex justify-between items-center px-2 cursor-pointer text-gray-600 hover:bg-emerald-100 duration-100' + (log.index % 2 !== 0 ? ' bg-slate-200/70' : '')}>
