export default function UserStudy(props) {
  ...
  return (
    <div className={ 'w-2/3 flex justify-between items-center py-1 px-3 rounded' + (index % 2 !== 0 ? ' bg-slate-100/70' : '') }>
      ...
      <div className="w-16 text-sm ml-8 font-normal">
        <div className="relative w-12 h-12 float-right rounded-full border-2 border-emerald-400">
          <p className="absolute left-2.5 top-1">{ completed_qs + (completed ? 0 : 1) }</p>
          <div className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-0.5 -rotate-45 bg-emerald-400"></div>
          <p className="absolute right-2.5 bottom-1">{ type === 0 ? '6' : '9' }</p>
        </div>
      </div>
    </div>
  )
